    ...
    T: Ord + Copy + fmt::Debug,
{
    ...
    while read_pointer <= end && array.read().unwrap()[read_pointer] < array.read().unwrap()[0] {
        new_output[new_output_pointer] = array.read().unwrap()[read_pointer];
        new_output_pointer += 1;
        read_pointer += 1;
    }

    new_output[new_output_pointer] = array.read().unwrap()[0];
    ...
    let input_length = {
        ...
        for i in 1..input_length {
            let value = array.read().unwrap()[start + i];

            if new_output[sublist_last_pointer] < value {
                while read_pointer <= end && array.read().unwrap()[read_pointer] < value {
                    new_output[new_output_pointer] = array.read().unwrap()[read_pointer];
                    new_output_pointer += 1;
                    read_pointer += 1;
                }
                ...
                array.write().unwrap()[start + new_input_length] = value;
                new_input_length += 1;
            }
        }
    ...
    for i in 0..new_output_pointer {
        array.write().unwrap()[start + input_length + i] = new_output[i];
    }
