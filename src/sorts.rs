...
/*use self::{
    bubble::*, coctail_shaker::*, counting::*, gnome::*, heap::*, hoare_quick::*,
    in_place_merge::*, in_place_quick::*, insertion::*, intro::*, merge::*, quick::*, selection::*,
    strand::*, tim::*, weave_merge::*,
}; */

use self::{bubble::*, heap::*, strand::*};
...
pub type Arr<T> = Arc<RwLock<ArrayWithCounters<T>>>;
...
pub fn run_sort(dictionary: &SortsDictionary, sort: Sort, array: &mut Arr<Item>) {
    let len = array.read().unwrap().len() - 1;
    dictionary.get(&sort).unwrap()(array, 0, len)
}
